...
function importFilesAndShow_poverty_people() {
  const baseUrl = `${
    window.charts_path
  }/${"poverty_people"}/${"poverty_people"}`;
  jQuery.get(baseUrl + "_1.9.csv", function(povcal_1_9) {
    window.chart_data["poverty_people"].data["1_9"] = fromCSV(
      povcal_1_9,
      ["string"].concat(Array(5).fill("number"))
    );
    jQuery.get(baseUrl + "_7.5.csv", function(povcal_7_5) {
      window.chart_data["poverty_people"].data["7_5"] = fromCSV(
        povcal_7_5,
        ["string"].concat(Array(5).fill("number"))
      );
      jQuery.get(baseUrl + "_10.csv", function(povcal_10) {
        window.chart_data["poverty_people"].data["10"] = fromCSV(
          povcal_10,
          ["string"].concat(Array(5).fill("number"))
        );
        jQuery.get(baseUrl + "_15.csv", function(povcal_15) {
          window.chart_data["poverty_people"].data["15"] = fromCSV(
            povcal_15,
            ["string"].concat(Array(5).fill("number"))
          );
