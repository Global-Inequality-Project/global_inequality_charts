...
function importFilesAndShow_eco_breakdown_material_footprint_overshoot() {
  loadCsv(
    `${
      window.charts_path
    }/${"eco_breakdown_material_footprint_overshoot"}/${"eco_breakdown_material_footprint_overshoot"}.csv`,
    function(err, overshot_data) {
      if (err === null) {
        loadCsv(
          `${
            window.charts_path
          }/${"eco_breakdown_material_footprint_overshoot"}/country.csv`,
          function(err, countries) {
            if (err === null) {
              window.chart_data[
                "eco_breakdown_material_footprint_overshoot"
              ].data = fromCSV(
                overshot_data,
...
function render_eco_breakdown_material_footprint_overshoot(canvasID) {
  ...
  let sorted = window.chart_data[chartID].data.sort(
    (a, b) => b["Cumulative-MF-overshoot"] - a["Cumulative-MF-overshoot"]
  );
  //sorted = sorted.filter(x => x.iso != 'GUY'); // EXCLUDING Guyana
  let countries = [];
  let series = [{ name: "Gigatonnes (Gt)", data: [] }];
  sorted.forEach((row) => {
  ...
  var options = {
    ...
    xaxis: {
      ...
      labels: {
        formatter: (val, index) => formatYAxisLabel(val, index, 0) + " Gt",
      },
      axisTicks: { height: 0 },
    ...
    tooltip: {
      y: {
        formatter: (val, index) => formatTooltipVal(val, index) + ' Gt',
      },
      shared: true,
      intersect: false,
    },
  ...
  options["chart"].id = "Cumulative Material Footprint overshoot".replace(
    / /g,
    ""
  );
