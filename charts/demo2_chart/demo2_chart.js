...
function prepare_demo2_chart() {
  loadJson(`${window.charts_path}/demo2_chart/custom_data_file.json`, function(
    error,
    data
  ) {
    if (error === null) {
...
function render_demo2_chart(canvasID, modal) {
  ...
  var options = {
    chart: {
      type: "line",
      height: "100%", // In most cases, this should be set to '100%'
  ...
  return [
    createApexChart(canvasID, options, callback),
