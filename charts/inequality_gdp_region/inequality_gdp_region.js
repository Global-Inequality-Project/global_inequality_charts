...
function importFilesAndShow_inequality_gdp_region() {
  jQuery.get(
    `${
      window.charts_path
    }/${"inequality_gdp_region"}/${"inequality_gdp_region"}.csv`,
    function(gdp_region) {
      window.chart_data["inequality_gdp_region"].data = fromCSV(
        gdp_region,
...
function render_inequality_gdp_region(canvasID) {
      ...
      },
      {
        ...
        options: {
          yaxis: {
            ...
            labels: {
              formatter: (val, index) =>
                "$" + formatYAxisLabel(val, index, 0, true),
            },
          },
          tooltip: {
  ...
  let years = [];
  for (
    let year = window.chart_data["inequality_gdp_region"].years.start,
      end = window.chart_data["inequality_gdp_region"].years.end;
    year <= end;
    ++year
  )
    years.push(year);
  ...
  years.forEach((year) => {
    ...
    if (data_row) {
      window.chart_data["inequality_gdp_region"].regions.forEach(
        (regionObj, i) => {
