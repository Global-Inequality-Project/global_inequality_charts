...
function importFilesAndShow_poverty_num_min_china() {
  ...
  jQuery.get(`${window.charts_path}/${chartID}/${chartID}.csv`, function(
    poverty_num_min_china
  ) {
    window.chart_data[chartID].data = fromCSV(poverty_num_min_china, [
      "string",
      "number",
      "number",
      "number",
...
function render_poverty_num_min_china(canvasID) {
  ...
  let years = [];
  for (
    let year = window.chart_data["poverty_num_min_china"].years.start,
      end = window.chart_data["poverty_num_min_china"].years.end;
    year <= end;
    ++year
  )
    years.push(year);
