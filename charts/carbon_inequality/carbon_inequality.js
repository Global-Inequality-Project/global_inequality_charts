...
function importFilesAndShow_carbon_inequality(){
	jQuery.get(`${window.charts_path}/${"carbon_inequality"}/${"carbon_inequality"}.csv`, function(gdp_nrt_sth){
        window.chart_data["carbon_inequality"].data = fromCSV(gdp_nrt_sth, ['string', 'number', 'number']);
...
function render_carbon_inequality(canvasID) {
    ...
    var options = {
        ...
        yaxis: {
            ...
            tickAmount: 5,
            labels: {
                formatter: (val, index) => formatYAxisLabel(val, index, 0, true)+' TpC',
                maxWidth: 40,
            }
        },
        grid: {
	...
	let percentiles = [];
	for (let percentile=window.chart_data["carbon_inequality"].percentiles.start, end = window.chart_data["carbon_inequality"].percentiles.end; percentile <= end; ++percentile)
    ...
    percentiles.forEach(percentile => {
        ...
        if (data_row)
            series[0].data.push(data_row['co2_emissions']);
        else
